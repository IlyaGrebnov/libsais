...
static LIBSAIS_CONTEXT * libsais_create_ctx_main(int threads)
{
...
static void libsais_place_cached_suffixes(int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
...
static void libsais_compact_and_place_cached_suffixes(int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_accumulate_counts_s32_2(int * RESTRICT bucket00, ptrdiff_t bucket_size, ptrdiff_t bucket_stride)
{
    int * RESTRICT bucket01 = bucket00 - bucket_stride;
    ptrdiff_t s; for (s = 0; s < bucket_size; s += 1) { bucket00[s] = bucket00[s] + bucket01[s]; }
}

static void libsais_accumulate_counts_s32_3(int * RESTRICT bucket00, ptrdiff_t bucket_size, ptrdiff_t bucket_stride)
{
    int * RESTRICT bucket01 = bucket00 - bucket_stride;
    int * RESTRICT bucket02 = bucket01 - bucket_stride;
    ptrdiff_t s; for (s = 0; s < bucket_size; s += 1) { bucket00[s] = bucket00[s] + bucket01[s] + bucket02[s]; }
}

static void libsais_accumulate_counts_s32_4(int * RESTRICT bucket00, ptrdiff_t bucket_size, ptrdiff_t bucket_stride)
{
    int * RESTRICT bucket01 = bucket00 - bucket_stride;
    int * RESTRICT bucket02 = bucket01 - bucket_stride;
    int * RESTRICT bucket03 = bucket02 - bucket_stride;
    ptrdiff_t s; for (s = 0; s < bucket_size; s += 1) { bucket00[s] = bucket00[s] + bucket01[s] + bucket02[s] + bucket03[s]; }
}

static void libsais_accumulate_counts_s32_5(int * RESTRICT bucket00, ptrdiff_t bucket_size, ptrdiff_t bucket_stride)
{
    int * RESTRICT bucket01 = bucket00 - bucket_stride;
    int * RESTRICT bucket02 = bucket01 - bucket_stride;
    int * RESTRICT bucket03 = bucket02 - bucket_stride;
    int * RESTRICT bucket04 = bucket03 - bucket_stride;
    ptrdiff_t s; for (s = 0; s < bucket_size; s += 1) { bucket00[s] = bucket00[s] + bucket01[s] + bucket02[s] + bucket03[s] + bucket04[s]; }
}

static void libsais_accumulate_counts_s32_6(int * RESTRICT bucket00, ptrdiff_t bucket_size, ptrdiff_t bucket_stride)
{
    int * RESTRICT bucket01 = bucket00 - bucket_stride;
    int * RESTRICT bucket02 = bucket01 - bucket_stride;
    int * RESTRICT bucket03 = bucket02 - bucket_stride;
    int * RESTRICT bucket04 = bucket03 - bucket_stride;
    int * RESTRICT bucket05 = bucket04 - bucket_stride;
    ptrdiff_t s; for (s = 0; s < bucket_size; s += 1) { bucket00[s] = bucket00[s] + bucket01[s] + bucket02[s] + bucket03[s] + bucket04[s] + bucket05[s]; }
}

static void libsais_accumulate_counts_s32_7(int * RESTRICT bucket00, ptrdiff_t bucket_size, ptrdiff_t bucket_stride)
{
    int * RESTRICT bucket01 = bucket00 - bucket_stride;
    int * RESTRICT bucket02 = bucket01 - bucket_stride;
    int * RESTRICT bucket03 = bucket02 - bucket_stride;
    int * RESTRICT bucket04 = bucket03 - bucket_stride;
    int * RESTRICT bucket05 = bucket04 - bucket_stride;
    int * RESTRICT bucket06 = bucket05 - bucket_stride;
    ptrdiff_t s; for (s = 0; s < bucket_size; s += 1) { bucket00[s] = bucket00[s] + bucket01[s] + bucket02[s] + bucket03[s] + bucket04[s] + bucket05[s] + bucket06[s]; }
}

static void libsais_accumulate_counts_s32_8(int * RESTRICT bucket00, ptrdiff_t bucket_size, ptrdiff_t bucket_stride)
{
    int * RESTRICT bucket01 = bucket00 - bucket_stride;
    int * RESTRICT bucket02 = bucket01 - bucket_stride;
    int * RESTRICT bucket03 = bucket02 - bucket_stride;
    ...
    int * RESTRICT bucket05 = bucket04 - bucket_stride;
    int * RESTRICT bucket06 = bucket05 - bucket_stride;
    int * RESTRICT bucket07 = bucket06 - bucket_stride;
    ptrdiff_t s; for (s = 0; s < bucket_size; s += 1) { bucket00[s] = bucket00[s] + bucket01[s] + bucket02[s] + bucket03[s] + bucket04[s] + bucket05[s] + bucket06[s] + bucket07[s]; }
}

static void libsais_accumulate_counts_s32_9(int * RESTRICT bucket00, ptrdiff_t bucket_size, ptrdiff_t bucket_stride)
{
    int * RESTRICT bucket01 = bucket00 - bucket_stride;
    int * RESTRICT bucket02 = bucket01 - bucket_stride;
    int * RESTRICT bucket03 = bucket02 - bucket_stride;
    ...
    int * RESTRICT bucket06 = bucket05 - bucket_stride;
    int * RESTRICT bucket07 = bucket06 - bucket_stride;
    int * RESTRICT bucket08 = bucket07 - bucket_stride;
    ptrdiff_t s; for (s = 0; s < bucket_size; s += 1) { bucket00[s] = bucket00[s] + bucket01[s] + bucket02[s] + bucket03[s] + bucket04[s] + bucket05[s] + bucket06[s] + bucket07[s] + bucket08[s]; }
}

static void libsais_accumulate_counts_s32(int * RESTRICT buckets, ptrdiff_t bucket_size, ptrdiff_t bucket_stride, ptrdiff_t num_buckets)
{
    while (num_buckets >= 9)
    {
...
static int libsais_count_and_gather_lms_suffixes_32s_4k(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    memset(buckets, 0, 4 * (size_t)k * sizeof(int));
...
static int libsais_count_and_gather_lms_suffixes_32s_2k(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    memset(buckets, 0, 2 * (size_t)k * sizeof(int));
...
static int libsais_count_and_gather_compacted_lms_suffixes_32s_2k(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    memset(buckets, 0, 2 * (size_t)k * sizeof(int));
    ...
    if (omp_block_size > 0)
    {
        ...
        for (i = m - 1, j = omp_block_start + prefetch_distance + 3; i >= j; i -= 4)
        {
            ...
            c1 = T[i - 0]; s = (s << 1) + (size_t)(c1 > (c0 - (ptrdiff_t)(s & 1))); SA[m] = (int)(i + 1); m -= ((ptrdiff_t)(s & 3) == (c0 >= 0));
            c0 &= INT_MAX; buckets[BUCKETS_INDEX2((size_t)c0, (s & 3) == 1)]++;

            c0 = T[i - 1]; s = (s << 1) + (size_t)(c0 > (c1 - (ptrdiff_t)(s & 1))); SA[m] = (int)(i - 0); m -= ((ptrdiff_t)(s & 3) == (c1 >= 0));
            c1 &= INT_MAX; buckets[BUCKETS_INDEX2((size_t)c1, (s & 3) == 1)]++;

            c1 = T[i - 2]; s = (s << 1) + (size_t)(c1 > (c0 - (ptrdiff_t)(s & 1))); SA[m] = (int)(i - 1); m -= ((ptrdiff_t)(s & 3) == (c0 >= 0));
            c0 &= INT_MAX; buckets[BUCKETS_INDEX2((size_t)c0, (s & 3) == 1)]++;

            c0 = T[i - 3]; s = (s << 1) + (size_t)(c0 > (c1 - (ptrdiff_t)(s & 1))); SA[m] = (int)(i - 2); m -= ((ptrdiff_t)(s & 3) == (c1 >= 0));
            c1 &= INT_MAX; buckets[BUCKETS_INDEX2((size_t)c1, (s & 3) == 1)]++;
        }

        for (j -= prefetch_distance + 3; i >= j; i -= 1)
        {
            c1 = c0; c0 = T[i]; s = (s << 1) + (size_t)(c0 > (c1 - (ptrdiff_t)(s & 1))); SA[m] = (int)(i + 1); m -= ((ptrdiff_t)(s & 3) == (c1 >= 0));
            c1 &= INT_MAX; buckets[BUCKETS_INDEX2((size_t)c1, (s & 3) == 1)]++;
        }

        c1 = (i >= 0) ? T[i] : -1; s = (s << 1) + (size_t)(c1 > (c0 - (ptrdiff_t)(s & 1))); SA[m] = (int)(i + 1); m -= ((ptrdiff_t)(s & 3) == (c0 >= 0));
        c0 &= INT_MAX; buckets[BUCKETS_INDEX2((size_t)c0, (s & 3) == 1)]++;
    }
...
static ptrdiff_t libsais_get_bucket_stride(ptrdiff_t free_space, ptrdiff_t bucket_size, ptrdiff_t num_buckets)
{
    ptrdiff_t bucket_size_1024 = (bucket_size + 1023) & (-1024); if (free_space / (num_buckets - 1) >= bucket_size_1024) { return bucket_size_1024; }
    ptrdiff_t bucket_size_16 = (bucket_size + 15) & (-16); if (free_space / (num_buckets - 1) >= bucket_size_16) { return bucket_size_16; }

    return bucket_size;
}

static int libsais_count_and_gather_lms_suffixes_32s_4k_fs_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    int m = 0;
...
#endif
    {
        ...
        {
            ptrdiff_t bucket_size       = 4 * (ptrdiff_t)k;
            ptrdiff_t bucket_stride     = libsais_get_bucket_stride(buckets - &SA[n], bucket_size, omp_num_threads);
            ...
            if (omp_thread_num == omp_num_threads - 1)
            {
                ...
                for (t = omp_num_threads - 1; t >= 0; --t)
                {
                    m += (int)thread_state[t].state.count;

                    if (t != omp_num_threads - 1 && thread_state[t].state.count > 0)
                    {
                        memcpy(&SA[n - m], &SA[thread_state[t].state.position - thread_state[t].state.count], (size_t)thread_state[t].state.count * sizeof(int));
                    }
                }
            }
            else
...
static int libsais_count_and_gather_lms_suffixes_32s_2k_fs_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    int m = 0;
...
#endif
    {
        ...
        {
            ptrdiff_t bucket_size       = 2 * (ptrdiff_t)k;
            ptrdiff_t bucket_stride     = libsais_get_bucket_stride(buckets - &SA[n], bucket_size, omp_num_threads);
...
static void libsais_count_and_gather_compacted_lms_suffixes_32s_2k_fs_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && n >= 65536)
#endif
    {
        ...
        {
            ptrdiff_t bucket_size       = 2 * (ptrdiff_t)k;
            ptrdiff_t bucket_stride     = libsais_get_bucket_stride(buckets - &SA[n + n], bucket_size, omp_num_threads);
            ...
            {
                ptrdiff_t t, m = 0; for (t = omp_num_threads - 1; t >= omp_thread_num; --t) { m += (int)thread_state[t].state.count; }

                if (thread_state[omp_thread_num].state.count > 0)
                {
                    memcpy(&SA[n - m], &SA[n + thread_state[omp_thread_num].state.position - thread_state[omp_thread_num].state.count], (size_t)thread_state[omp_thread_num].state.count * sizeof(int));
                }
            }
...
static int libsais_count_and_gather_lms_suffixes_32s_4k_nofs_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads)
{
    int m = 0;
...
static int libsais_count_and_gather_lms_suffixes_32s_2k_nofs_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads)
{
    int m = 0;
...
static int libsais_count_and_gather_compacted_lms_suffixes_32s_2k_nofs_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads)
{
    int m = 0;
...
static int libsais_count_and_gather_lms_suffixes_32s_4k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    int m;

#if defined(_OPENMP)
    int max_threads = (int)((buckets - &SA[n]) / ((4 * (ptrdiff_t)k + 15) & (-16))); if (max_threads > threads) { max_threads = threads; }
...
static int libsais_count_and_gather_lms_suffixes_32s_2k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    int m;

#if defined(_OPENMP)
    int max_threads = (int)((buckets - &SA[n]) / ((2 * (ptrdiff_t)k + 15) & (-16))); if (max_threads > threads) { max_threads = threads; }
    if (max_threads > 1 && n >= 65536 && n / k >= 2)
    {
...
static void libsais_count_and_gather_compacted_lms_suffixes_32s_2k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
#if defined(_OPENMP)
    int max_threads = (int)((buckets - &SA[n + n]) / ((2 * (ptrdiff_t)k + 15) & (-16))); if (max_threads > threads) { max_threads = threads; }
    if (max_threads > 1 && n >= 65536 && n / k >= 2)
    {
        if (max_threads > n / 8 / k) { max_threads = n / 8 / k; }
...
static void libsais_radix_sort_lms_suffixes_32s_6k(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT induction_bucket, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_radix_sort_lms_suffixes_32s_2k(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT induction_bucket, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_radix_sort_lms_suffixes_32s_block_gather(const int * RESTRICT T, int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_radix_sort_lms_suffixes_32s_6k_block_sort(int * RESTRICT induction_bucket, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_radix_sort_lms_suffixes_32s_2k_block_sort(int * RESTRICT induction_bucket, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_radix_sort_lms_suffixes_32s_6k_block_omp(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT induction_bucket, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
...
static void libsais_radix_sort_lms_suffixes_32s_2k_block_omp(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT induction_bucket, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && block_size >= 16384)
#endif
...
static void libsais_radix_sort_lms_suffixes_32s_6k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int m, int * RESTRICT induction_bucket, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    if (threads == 1 || m < 65536)
    {
        libsais_radix_sort_lms_suffixes_32s_6k(T, SA, induction_bucket, (ptrdiff_t)n - (ptrdiff_t)m + 1, (ptrdiff_t)m - 1);
    }
#if defined(_OPENMP)
    else
    {
        ptrdiff_t block_start, block_end;
        for (block_start = 0; block_start < (ptrdiff_t)m - 1; block_start = block_end)
        {
            block_end = block_start + (ptrdiff_t)threads * LIBSAIS_PER_THREAD_CACHE_SIZE; if (block_end >= m) { block_end = (ptrdiff_t)m - 1; }

            libsais_radix_sort_lms_suffixes_32s_6k_block_omp(T, SA, induction_bucket, thread_state[0].state.cache, (ptrdiff_t)n - block_end, block_end - block_start, threads);
        }
    }
#else
    UNUSED(thread_state);
...
static void libsais_radix_sort_lms_suffixes_32s_2k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int m, int * RESTRICT induction_bucket, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    if (threads == 1 || m < 65536)
    {
        libsais_radix_sort_lms_suffixes_32s_2k(T, SA, induction_bucket, (ptrdiff_t)n - (ptrdiff_t)m + 1, (ptrdiff_t)m - 1);
    }
#if defined(_OPENMP)
    else
    {
        ...
        for (block_start = 0; block_start < (ptrdiff_t)m - 1; block_start = block_end)
        {
            ...
            libsais_radix_sort_lms_suffixes_32s_2k_block_omp(T, SA, induction_bucket, thread_state[0].state.cache, (ptrdiff_t)n - block_end, block_end - block_start, threads);
        }
    }
#else
    UNUSED(thread_state);
...
static int libsais_partial_sorting_scan_left_to_right_8u_omp(const unsigned char * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT buckets, int left_suffixes_count, int d, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    ...
    else
    {
        ...
        for (block_start = 0; block_start < left_suffixes_count; )
        {
            ...
            {
                ptrdiff_t block_max_end = block_start + ((ptrdiff_t)threads) * (LIBSAIS_PER_THREAD_CACHE_SIZE - 16 * (ptrdiff_t)threads); if (block_max_end > left_suffixes_count) { block_max_end = left_suffixes_count;}
...
static int libsais_partial_sorting_scan_left_to_right_32s_6k(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT buckets, int d, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static int libsais_partial_sorting_scan_left_to_right_32s_4k(const int * RESTRICT T, int * RESTRICT SA, int k, int * RESTRICT buckets, int d, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_partial_sorting_scan_left_to_right_32s_1k(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT induction_bucket, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_partial_sorting_scan_left_to_right_32s_6k_block_gather(const int * RESTRICT T, int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_start + omp_block_size - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int p0 = cache[i + 0].index = SA[i + 0]; int symbol0 = 0; p0 &= INT_MAX; if (p0 != 0) { symbol0 = BUCKETS_INDEX4(T[p0 - 1], T[p0 - 2] >= T[p0 - 1]); } cache[i + 0].symbol = symbol0;
        int p1 = cache[i + 1].index = SA[i + 1]; int symbol1 = 0; p1 &= INT_MAX; if (p1 != 0) { symbol1 = BUCKETS_INDEX4(T[p1 - 1], T[p1 - 2] >= T[p1 - 1]); } cache[i + 1].symbol = symbol1;
    }

    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        int p = cache[i].index = SA[i]; int symbol = 0; p &= INT_MAX; if (p != 0) { symbol = BUCKETS_INDEX4(T[p - 1], T[p - 2] >= T[p - 1]); } cache[i].symbol = symbol;
    }
}

static void libsais_partial_sorting_scan_left_to_right_32s_4k_block_gather(const int * RESTRICT T, int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_start + omp_block_size - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int s0 = SA[i + prefetch_distance + 0]; const int * Ts0 = &T[s0 & ~SUFFIX_GROUP_MARKER] - 1; libsais_prefetch(s0 > 0 ? Ts0 : NULL); Ts0--; libsais_prefetch(s0 > 0 ? Ts0 : NULL);
        int s1 = SA[i + prefetch_distance + 1]; const int * Ts1 = &T[s1 & ~SUFFIX_GROUP_MARKER] - 1; libsais_prefetch(s1 > 0 ? Ts1 : NULL); Ts1--; libsais_prefetch(s1 > 0 ? Ts1 : NULL);

        libsais_prefetchw(&cache[i + prefetch_distance]);

        int symbol0 = INT_MIN, p0 = SA[i + 0]; if (p0 > 0) { cache[i + 0].index = p0; p0 &= ~SUFFIX_GROUP_MARKER; symbol0 = BUCKETS_INDEX2(T[p0 - 1], T[p0 - 2] < T[p0 - 1]); p0 = 0; } cache[i + 0].symbol = symbol0; SA[i + 0] = p0 & INT_MAX;
        int symbol1 = INT_MIN, p1 = SA[i + 1]; if (p1 > 0) { cache[i + 1].index = p1; p1 &= ~SUFFIX_GROUP_MARKER; symbol1 = BUCKETS_INDEX2(T[p1 - 1], T[p1 - 2] < T[p1 - 1]); p1 = 0; } cache[i + 1].symbol = symbol1; SA[i + 1] = p1 & INT_MAX;
    }

    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        int symbol = INT_MIN, p = SA[i]; if (p > 0) { cache[i].index = p; p &= ~SUFFIX_GROUP_MARKER; symbol = BUCKETS_INDEX2(T[p - 1], T[p - 2] < T[p - 1]); p = 0; } cache[i].symbol = symbol; SA[i] = p & INT_MAX;
    }
}

static void libsais_partial_sorting_scan_left_to_right_32s_1k_block_gather(const int * RESTRICT T, int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_start + omp_block_size - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int s0 = SA[i + prefetch_distance + 0]; const int * Ts0 = &T[s0] - 1; libsais_prefetch(s0 > 0 ? Ts0 : NULL); Ts0--; libsais_prefetch(s0 > 0 ? Ts0 : NULL);
        int s1 = SA[i + prefetch_distance + 1]; const int * Ts1 = &T[s1] - 1; libsais_prefetch(s1 > 0 ? Ts1 : NULL); Ts1--; libsais_prefetch(s1 > 0 ? Ts1 : NULL);

        libsais_prefetchw(&cache[i + prefetch_distance]);

        int symbol0 = INT_MIN, p0 = SA[i + 0]; if (p0 > 0) { cache[i + 0].index = (p0 - 1) | ((T[p0 - 2] < T[p0 - 1]) << (INT_BIT - 1)); symbol0 = T[p0 - 1]; p0 = 0; } cache[i + 0].symbol = symbol0; SA[i + 0] = p0 & INT_MAX;
        int symbol1 = INT_MIN, p1 = SA[i + 1]; if (p1 > 0) { cache[i + 1].index = (p1 - 1) | ((T[p1 - 2] < T[p1 - 1]) << (INT_BIT - 1)); symbol1 = T[p1 - 1]; p1 = 0; } cache[i + 1].symbol = symbol1; SA[i + 1] = p1 & INT_MAX;
    }

    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        int symbol = INT_MIN, p = SA[i]; if (p > 0) { cache[i].index = (p - 1) | ((T[p - 2] < T[p - 1]) << (INT_BIT - 1)); symbol = T[p - 1]; p = 0; } cache[i].symbol = symbol; SA[i] = p & INT_MAX;
    }
}

static int libsais_partial_sorting_scan_left_to_right_32s_6k_block_sort(const int * RESTRICT T, int * RESTRICT buckets, int d, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;

    ptrdiff_t i, j, omp_block_end = omp_block_start + omp_block_size;
    for (i = omp_block_start, j = omp_block_end - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int v0 = cache[i + 0].symbol, p0 = cache[i + 0].index; d += (p0 < 0); cache[i + 0].symbol = buckets[v0]++; cache[i + 0].index = (p0 - 1) | ((buckets[2 + v0] != d) << (INT_BIT - 1)); buckets[2 + v0] = d;
        if (cache[i + 0].symbol < omp_block_end) { int s = cache[i + 0].symbol, q = (cache[s].index = cache[i + 0].index) & INT_MAX; cache[s].symbol = BUCKETS_INDEX4(T[q - 1], T[q - 2] >= T[q - 1]); }

        int v1 = cache[i + 1].symbol, p1 = cache[i + 1].index; d += (p1 < 0); cache[i + 1].symbol = buckets[v1]++; cache[i + 1].index = (p1 - 1) | ((buckets[2 + v1] != d) << (INT_BIT - 1)); buckets[2 + v1] = d;
        if (cache[i + 1].symbol < omp_block_end) { int s = cache[i + 1].symbol, q = (cache[s].index = cache[i + 1].index) & INT_MAX; cache[s].symbol = BUCKETS_INDEX4(T[q - 1], T[q - 2] >= T[q - 1]); }
    }

    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        int v = cache[i].symbol, p = cache[i].index; d += (p < 0); cache[i].symbol = buckets[v]++; cache[i].index = (p - 1) | ((buckets[2 + v] != d) << (INT_BIT - 1)); buckets[2 + v] = d;
        if (cache[i].symbol < omp_block_end) { int s = cache[i].symbol, q = (cache[s].index = cache[i].index) & INT_MAX; cache[s].symbol = BUCKETS_INDEX4(T[q - 1], T[q - 2] >= T[q - 1]); }
    }

    return d;
}

static int libsais_partial_sorting_scan_left_to_right_32s_4k_block_sort(const int * RESTRICT T, int k, int * RESTRICT buckets, int d, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_end - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int s0 = cache[i + prefetch_distance + 0].symbol; const int * Is0 = &induction_bucket[s0 >> 1]; libsais_prefetchw(s0 >= 0 ? Is0 : NULL); const int * Ds0 = &distinct_names[s0]; libsais_prefetchw(s0 >= 0 ? Ds0 : NULL); 
        int s1 = cache[i + prefetch_distance + 1].symbol; const int * Is1 = &induction_bucket[s1 >> 1]; libsais_prefetchw(s1 >= 0 ? Is1 : NULL); const int * Ds1 = &distinct_names[s1]; libsais_prefetchw(s1 >= 0 ? Ds1 : NULL);
        
        int v0 = cache[i + 0].symbol;
        if (v0 >= 0)
        {
            int p0 = cache[i + 0].index; d += (p0 >> (SUFFIX_GROUP_BIT - 1)); cache[i + 0].symbol = induction_bucket[v0 >> 1]++; cache[i + 0].index = (p0 - 1) | (v0 << (INT_BIT - 1)) | ((distinct_names[v0] != d) << (SUFFIX_GROUP_BIT - 1)); distinct_names[v0] = d;
            if (cache[i + 0].symbol < omp_block_end) { int ni = cache[i + 0].symbol, np = cache[i + 0].index; if (np > 0) { cache[ni].index = np; np &= ~SUFFIX_GROUP_MARKER; cache[ni].symbol = BUCKETS_INDEX2(T[np - 1], T[np - 2] < T[np - 1]); np = 0; } cache[i + 0].index = np & INT_MAX; }
        }

        int v1 = cache[i + 1].symbol;
        if (v1 >= 0)
        {
            int p1 = cache[i + 1].index; d += (p1 >> (SUFFIX_GROUP_BIT - 1)); cache[i + 1].symbol = induction_bucket[v1 >> 1]++; cache[i + 1].index = (p1 - 1) | (v1 << (INT_BIT - 1)) | ((distinct_names[v1] != d) << (SUFFIX_GROUP_BIT - 1)); distinct_names[v1] = d;
            if (cache[i + 1].symbol < omp_block_end) { int ni = cache[i + 1].symbol, np = cache[i + 1].index; if (np > 0) { cache[ni].index = np; np &= ~SUFFIX_GROUP_MARKER; cache[ni].symbol = BUCKETS_INDEX2(T[np - 1], T[np - 2] < T[np - 1]); np = 0; } cache[i + 1].index = np & INT_MAX; }
        }
    }

    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        int v = cache[i].symbol;
        if (v >= 0)
        {
            int p = cache[i].index; d += (p >> (SUFFIX_GROUP_BIT - 1)); cache[i].symbol = induction_bucket[v >> 1]++; cache[i].index = (p - 1) | (v << (INT_BIT - 1)) | ((distinct_names[v] != d) << (SUFFIX_GROUP_BIT - 1)); distinct_names[v] = d;
            if (cache[i].symbol < omp_block_end) { int ni = cache[i].symbol, np = cache[i].index; if (np > 0) { cache[ni].index = np; np &= ~SUFFIX_GROUP_MARKER; cache[ni].symbol = BUCKETS_INDEX2(T[np - 1], T[np - 2] < T[np - 1]); np = 0; } cache[i].index = np & INT_MAX; }
        }
    }
...
static void libsais_partial_sorting_scan_left_to_right_32s_1k_block_sort(const int * RESTRICT T, int * RESTRICT induction_bucket, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_end - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int s0 = cache[i + prefetch_distance + 0].symbol; const int * Is0 = &induction_bucket[s0]; libsais_prefetchw(s0 >= 0 ? Is0 : NULL);
        int s1 = cache[i + prefetch_distance + 1].symbol; const int * Is1 = &induction_bucket[s1]; libsais_prefetchw(s1 >= 0 ? Is1 : NULL);
        
        int v0 = cache[i + 0].symbol;
        if (v0 >= 0)
        {
            cache[i + 0].symbol = induction_bucket[v0]++;
            if (cache[i + 0].symbol < omp_block_end) { int ni = cache[i + 0].symbol, np = cache[i + 0].index; if (np > 0) { cache[ni].index = (np - 1) | ((T[np - 2] < T[np - 1]) << (INT_BIT - 1)); cache[ni].symbol = T[np - 1]; np = 0; } cache[i + 0].index = np & INT_MAX; }
        }
        ...
        if (v1 >= 0)
        {
            ...
            if (cache[i + 1].symbol < omp_block_end) { int ni = cache[i + 1].symbol, np = cache[i + 1].index; if (np > 0) { cache[ni].index = (np - 1) | ((T[np - 2] < T[np - 1]) << (INT_BIT - 1)); cache[ni].symbol = T[np - 1]; np = 0; } cache[i + 1].index = np & INT_MAX; }
        }
    }
    ...
    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        ...
        if (v >= 0)
        {
            ...
            if (cache[i].symbol < omp_block_end) { int ni = cache[i].symbol, np = cache[i].index; if (np > 0) { cache[ni].index = (np - 1) | ((T[np - 2] < T[np - 1]) << (INT_BIT - 1)); cache[ni].symbol = T[np - 1]; np = 0; } cache[i].index = np & INT_MAX; }
        }
    }
}

static int libsais_partial_sorting_scan_left_to_right_32s_6k_block_omp(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT buckets, int d, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && block_size >= 16384)
#endif
...
static int libsais_partial_sorting_scan_left_to_right_32s_4k_block_omp(const int * RESTRICT T, int * RESTRICT SA, int k, int * RESTRICT buckets, int d, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && block_size >= 16384)
#endif
...
static void libsais_partial_sorting_scan_left_to_right_32s_1k_block_omp(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT buckets, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && block_size >= 16384)
#endif
...
static int libsais_partial_sorting_scan_left_to_right_32s_6k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT buckets, int left_suffixes_count, int d, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    ...
    else
    {
        ...
        {
            block_end = block_start + (ptrdiff_t)threads * LIBSAIS_PER_THREAD_CACHE_SIZE; if (block_end > left_suffixes_count) { block_end = left_suffixes_count; }
...
static int libsais_partial_sorting_scan_left_to_right_32s_4k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int d, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    int * RESTRICT induction_bucket = &buckets[2 * k];
    int * RESTRICT distinct_names   = &buckets[0 * k];
    ...
    else
    {
        ...
        {
            block_end = block_start + (ptrdiff_t)threads * LIBSAIS_PER_THREAD_CACHE_SIZE; if (block_end > n) { block_end = n; }
...
static void libsais_partial_sorting_scan_left_to_right_32s_1k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    SA[buckets[T[n - 1]]++] = (n - 1) | ((T[n - 2] < T[n - 1]) << (INT_BIT - 1));
...
static void libsais_partial_sorting_scan_right_to_left_8u_omp(const unsigned char * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT buckets, int first_lms_suffix, int left_suffixes_count, int d, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    ...
    else
    {
        ...
        for (block_start = scan_end - 1; block_start >= scan_start; )
        {
            ...
            {
                ptrdiff_t block_max_end = block_start - ((ptrdiff_t)threads) * (LIBSAIS_PER_THREAD_CACHE_SIZE - 16 * (ptrdiff_t)threads); if (block_max_end < scan_start) { block_max_end = scan_start - 1; }
                ptrdiff_t block_end     = block_start - 1; while (block_end > block_max_end && SA[block_end] != 0) { block_end--; }
...
static int libsais_partial_sorting_scan_right_to_left_32s_6k(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT buckets, int d, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static int libsais_partial_sorting_scan_right_to_left_32s_4k(const int * RESTRICT T, int * RESTRICT SA, int k, int * RESTRICT buckets, int d, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_partial_sorting_scan_right_to_left_32s_1k(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT induction_bucket, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_partial_sorting_scan_right_to_left_32s_6k_block_gather(const int * RESTRICT T, int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_partial_sorting_scan_right_to_left_32s_4k_block_gather(const int * RESTRICT T, int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_start + omp_block_size - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int symbol0 = INT_MIN, p0 = SA[i + 0]; if (p0 > 0) { SA[i + 0] = 0; cache[i + 0].index = p0; p0 &= ~SUFFIX_GROUP_MARKER; symbol0 = BUCKETS_INDEX2(T[p0 - 1], T[p0 - 2] > T[p0 - 1]); } cache[i + 0].symbol = symbol0;
        int symbol1 = INT_MIN, p1 = SA[i + 1]; if (p1 > 0) { SA[i + 1] = 0; cache[i + 1].index = p1; p1 &= ~SUFFIX_GROUP_MARKER; symbol1 = BUCKETS_INDEX2(T[p1 - 1], T[p1 - 2] > T[p1 - 1]); } cache[i + 1].symbol = symbol1;
    }

    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        int symbol = INT_MIN, p = SA[i]; if (p > 0) { SA[i] = 0; cache[i].index = p; p &= ~SUFFIX_GROUP_MARKER; symbol = BUCKETS_INDEX2(T[p - 1], T[p - 2] > T[p - 1]); } cache[i].symbol = symbol;
    }
}

static void libsais_partial_sorting_scan_right_to_left_32s_1k_block_gather(const int * RESTRICT T, int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_start + omp_block_size - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int symbol0 = INT_MIN, p0 = SA[i + 0]; if (p0 > 0) { SA[i + 0] = 0; cache[i + 0].index = (p0 - 1) | ((T[p0 - 2] > T[p0 - 1]) << (INT_BIT - 1)); symbol0 = T[p0 - 1]; } cache[i + 0].symbol = symbol0;
        int symbol1 = INT_MIN, p1 = SA[i + 1]; if (p1 > 0) { SA[i + 1] = 0; cache[i + 1].index = (p1 - 1) | ((T[p1 - 2] > T[p1 - 1]) << (INT_BIT - 1)); symbol1 = T[p1 - 1]; } cache[i + 1].symbol = symbol1;
    }

    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        int symbol = INT_MIN, p = SA[i]; if (p > 0) { SA[i] = 0; cache[i].index = (p - 1) | ((T[p - 2] > T[p - 1]) << (INT_BIT - 1)); symbol = T[p - 1]; } cache[i].symbol = symbol;
    }
}

static int libsais_partial_sorting_scan_right_to_left_32s_6k_block_sort(const int * RESTRICT T, int * RESTRICT buckets, int d, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start + omp_block_size - 1, j = omp_block_start + prefetch_distance + 1; i >= j; i -= 2)
    {
        ...
        int v0 = cache[i - 0].symbol, p0 = cache[i - 0].index; d += (p0 < 0); cache[i - 0].symbol = --buckets[v0]; cache[i - 0].index = (p0 - 1) | ((buckets[2 + v0] != d) << (INT_BIT - 1)); buckets[2 + v0] = d;
        if (cache[i - 0].symbol >= omp_block_start) { int s = cache[i - 0].symbol, q = (cache[s].index = cache[i - 0].index) & INT_MAX; cache[s].symbol = BUCKETS_INDEX4(T[q - 1], T[q - 2] > T[q - 1]); }

        int v1 = cache[i - 1].symbol, p1 = cache[i - 1].index; d += (p1 < 0); cache[i - 1].symbol = --buckets[v1]; cache[i - 1].index = (p1 - 1) | ((buckets[2 + v1] != d) << (INT_BIT - 1)); buckets[2 + v1] = d;
        if (cache[i - 1].symbol >= omp_block_start) { int s = cache[i - 1].symbol, q = (cache[s].index = cache[i - 1].index) & INT_MAX; cache[s].symbol = BUCKETS_INDEX4(T[q - 1], T[q - 2] > T[q - 1]); }
    }

    for (j -= prefetch_distance + 1; i >= j; i -= 1)
    {
        int v = cache[i].symbol, p = cache[i].index; d += (p < 0); cache[i].symbol = --buckets[v]; cache[i].index = (p - 1) | ((buckets[2 + v] != d) << (INT_BIT - 1)); buckets[2 + v] = d;
        if (cache[i].symbol >= omp_block_start) { int s = cache[i].symbol, q = (cache[s].index = cache[i].index) & INT_MAX; cache[s].symbol = BUCKETS_INDEX4(T[q - 1], T[q - 2] > T[q - 1]); }
    }

    return d;
}

static int libsais_partial_sorting_scan_right_to_left_32s_4k_block_sort(const int * RESTRICT T, int k, int * RESTRICT buckets, int d, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start + omp_block_size - 1, j = omp_block_start + prefetch_distance + 1; i >= j; i -= 2)
    {
        ...
        {
            int p0 = cache[i - 0].index; d += (p0 >> (SUFFIX_GROUP_BIT - 1)); cache[i - 0].symbol = --induction_bucket[v0 >> 1]; cache[i - 0].index = (p0 - 1) | (v0 << (INT_BIT - 1)) | ((distinct_names[v0] != d) << (SUFFIX_GROUP_BIT - 1)); distinct_names[v0] = d;
            if (cache[i - 0].symbol >= omp_block_start) { int ni = cache[i - 0].symbol, np = cache[i - 0].index; if (np > 0) { cache[i - 0].index = 0; cache[ni].index = np; np &= ~SUFFIX_GROUP_MARKER; cache[ni].symbol = BUCKETS_INDEX2(T[np - 1], T[np - 2] > T[np - 1]); } }
        }
        ...
        {
            int p1 = cache[i - 1].index; d += (p1 >> (SUFFIX_GROUP_BIT - 1)); cache[i - 1].symbol = --induction_bucket[v1 >> 1]; cache[i - 1].index = (p1 - 1) | (v1 << (INT_BIT - 1)) | ((distinct_names[v1] != d) << (SUFFIX_GROUP_BIT - 1)); distinct_names[v1] = d;
            if (cache[i - 1].symbol >= omp_block_start) { int ni = cache[i - 1].symbol, np = cache[i - 1].index; if (np > 0) { cache[i - 1].index = 0; cache[ni].index = np; np &= ~SUFFIX_GROUP_MARKER; cache[ni].symbol = BUCKETS_INDEX2(T[np - 1], T[np - 2] > T[np - 1]); } }
        }
    }
    ...
    for (j -= prefetch_distance + 1; i >= j; i -= 1)
    {
        ...
        {
            int p = cache[i].index; d += (p >> (SUFFIX_GROUP_BIT - 1)); cache[i].symbol = --induction_bucket[v >> 1]; cache[i].index = (p - 1) | (v << (INT_BIT - 1)) | ((distinct_names[v] != d) << (SUFFIX_GROUP_BIT - 1)); distinct_names[v] = d;
            if (cache[i].symbol >= omp_block_start) { int ni = cache[i].symbol, np = cache[i].index; if (np > 0) { cache[i].index = 0; cache[ni].index = np; np &= ~SUFFIX_GROUP_MARKER; cache[ni].symbol = BUCKETS_INDEX2(T[np - 1], T[np - 2] > T[np - 1]); } }
        }
    }
...
static void libsais_partial_sorting_scan_right_to_left_32s_1k_block_sort(const int * RESTRICT T, int * RESTRICT induction_bucket, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start + omp_block_size - 1, j = omp_block_start + prefetch_distance + 1; i >= j; i -= 2)
    {
        ...
        {
            cache[i - 0].symbol = --induction_bucket[v0];
            if (cache[i - 0].symbol >= omp_block_start) { int ni = cache[i - 0].symbol, np = cache[i - 0].index; if (np > 0) { cache[i - 0].index = 0; cache[ni].index = (np - 1) | ((T[np - 2] > T[np - 1]) << (INT_BIT - 1)); cache[ni].symbol = T[np - 1]; } }
        }
        ...
        {
            cache[i - 1].symbol = --induction_bucket[v1];
            if (cache[i - 1].symbol >= omp_block_start) { int ni = cache[i - 1].symbol, np = cache[i - 1].index; if (np > 0) { cache[i - 1].index = 0; cache[ni].index = (np - 1) | ((T[np - 2] > T[np - 1]) << (INT_BIT - 1)); cache[ni].symbol = T[np - 1]; }}
        }
    }
    ...
    for (j -= prefetch_distance + 1; i >= j; i -= 1)
    {
        ...
        {
            cache[i].symbol = --induction_bucket[v];
            if (cache[i].symbol >= omp_block_start) { int ni = cache[i].symbol, np = cache[i].index; if (np > 0) { cache[i].index = 0; cache[ni].index = (np - 1) | ((T[np - 2] > T[np - 1]) << (INT_BIT - 1)); cache[ni].symbol = T[np - 1]; } }
        }
    }
}

static int libsais_partial_sorting_scan_right_to_left_32s_6k_block_omp(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT buckets, int d, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && block_size >= 16384)
#endif
...
static int libsais_partial_sorting_scan_right_to_left_32s_4k_block_omp(const int * RESTRICT T, int * RESTRICT SA, int k, int * RESTRICT buckets, int d, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && block_size >= 16384)
#endif
...
static void libsais_partial_sorting_scan_right_to_left_32s_1k_block_omp(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT buckets, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && block_size >= 16384)
#endif
...
static int libsais_partial_sorting_scan_right_to_left_32s_6k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT buckets, int first_lms_suffix, int left_suffixes_count, int d, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    ptrdiff_t scan_start    = (ptrdiff_t)left_suffixes_count + 1;
    ptrdiff_t scan_end      = (ptrdiff_t)n - (ptrdiff_t)first_lms_suffix;
    ...
    else
    {
        ...
        {
            block_end = block_start - (ptrdiff_t)threads * LIBSAIS_PER_THREAD_CACHE_SIZE; if (block_end < scan_start) { block_end = scan_start - 1; }
...
static int libsais_partial_sorting_scan_right_to_left_32s_4k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int d, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    if (threads == 1 || n < 65536)
    {
    ...
    {
        ptrdiff_t block_start, block_end;
        for (block_start = (ptrdiff_t)n - 1; block_start >= 0; block_start = block_end)
        {
            block_end = block_start - (ptrdiff_t)threads * LIBSAIS_PER_THREAD_CACHE_SIZE; if (block_end < 0) { block_end = -1; }
...
static void libsais_partial_sorting_scan_right_to_left_32s_1k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    if (threads == 1 || n < 65536)
    {
...
static ptrdiff_t libsais_partial_sorting_gather_lms_suffixes_32s_4k(int * RESTRICT SA, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static ptrdiff_t libsais_partial_sorting_gather_lms_suffixes_32s_1k(int * RESTRICT SA, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_partial_sorting_gather_lms_suffixes_32s_4k_omp(int * RESTRICT SA, int n, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && n >= 65536)
#endif
...
static void libsais_partial_sorting_gather_lms_suffixes_32s_1k_omp(int * RESTRICT SA, int n, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && n >= 65536)
#endif
...
static void libsais_induce_partial_order_32s_6k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int first_lms_suffix, int left_suffixes_count, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    int d = libsais_partial_sorting_scan_left_to_right_32s_6k_omp(T, SA, n, buckets, left_suffixes_count, 0, threads, thread_state);
...
static void libsais_induce_partial_order_32s_4k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    memset(buckets, 0, 2 * (size_t)k * sizeof(int));

    int d = libsais_partial_sorting_scan_left_to_right_32s_4k_omp(T, SA, n, k, buckets, 0, threads, thread_state);
...
static void libsais_induce_partial_order_32s_2k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
...
static void libsais_induce_partial_order_32s_1k_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
...
static void libsais_mark_distinct_lms_suffixes_32s(int * RESTRICT SA, int m, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_clamp_lms_suffixes_length_32s(int * RESTRICT SA, int m, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_mark_distinct_lms_suffixes_32s_omp(int * RESTRICT SA, int n, int m, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && n >= 131072)
#endif
...
static void libsais_clamp_lms_suffixes_length_32s_omp(int * RESTRICT SA, int n, int m, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && n >= 131072)
#endif
...
static int libsais_renumber_and_mark_distinct_lms_suffixes_32s_1k_omp(int * RESTRICT T, int * RESTRICT SA, int n, int m, int threads)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_final_sorting_scan_left_to_right_32s(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT induction_bucket, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_final_sorting_scan_left_to_right_32s_block_gather(const int * RESTRICT T, int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_start + omp_block_size - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int symbol0 = INT_MIN, p0 = SA[i + 0]; SA[i + 0] = p0 ^ INT_MIN; if (p0 > 0) { p0--; cache[i + 0].index = p0 | (((T[p0 - (p0 > 0)] < T[p0])) << (INT_BIT - 1)); symbol0 = T[p0]; } cache[i + 0].symbol = symbol0;
        int symbol1 = INT_MIN, p1 = SA[i + 1]; SA[i + 1] = p1 ^ INT_MIN; if (p1 > 0) { p1--; cache[i + 1].index = p1 | (((T[p1 - (p1 > 0)] < T[p1])) << (INT_BIT - 1)); symbol1 = T[p1]; } cache[i + 1].symbol = symbol1;
    }

    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        int symbol = INT_MIN, p = SA[i]; SA[i] = p ^ INT_MIN; if (p > 0) { p--; cache[i].index = p | (((T[p - (p > 0)] < T[p])) << (INT_BIT - 1)); symbol = T[p]; } cache[i].symbol = symbol;
    }
}

static void libsais_final_sorting_scan_left_to_right_32s_block_sort(const int * RESTRICT T, int * RESTRICT induction_bucket, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_end - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        {
            cache[i + 0].symbol = induction_bucket[v0]++;
            if (cache[i + 0].symbol < omp_block_end) { int ni = cache[i + 0].symbol, np = cache[i + 0].index; cache[i + 0].index = np ^ INT_MIN; if (np > 0) { np--; cache[ni].index = np | (((T[np - (np > 0)] < T[np])) << (INT_BIT - 1)); cache[ni].symbol = T[np]; } }
        }
        ...
        {
            cache[i + 1].symbol = induction_bucket[v1]++;
            if (cache[i + 1].symbol < omp_block_end) { int ni = cache[i + 1].symbol, np = cache[i + 1].index; cache[i + 1].index = np ^ INT_MIN; if (np > 0) { np--; cache[ni].index = np | (((T[np - (np > 0)] < T[np])) << (INT_BIT - 1)); cache[ni].symbol = T[np]; } }
        }
    }
    ...
    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        ...
        {
            cache[i].symbol = induction_bucket[v]++;
            if (cache[i].symbol < omp_block_end) { int ni = cache[i].symbol, np = cache[i].index; cache[i].index = np ^ INT_MIN; if (np > 0) { np--; cache[ni].index = np | (((T[np - (np > 0)] < T[np])) << (INT_BIT - 1)); cache[ni].symbol = T[np]; } }
        }
    }
}
...
static void libsais_final_sorting_scan_left_to_right_32s_block_omp(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT buckets, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && block_size >= 16384)
#endif
...
static void libsais_final_bwt_scan_left_to_right_8u_omp(const unsigned char * RESTRICT T, int * RESTRICT SA, ptrdiff_t n, int * RESTRICT induction_bucket, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    ...
    else
    {
        ...
        for (block_start = 0; block_start < n; )
        {
            ...
            {
                ptrdiff_t block_max_end = block_start + ((ptrdiff_t)threads) * (LIBSAIS_PER_THREAD_CACHE_SIZE - 16 * (ptrdiff_t)threads); if (block_max_end > n) { block_max_end = n;}
                ptrdiff_t block_end     = block_start + 1; while (block_end < block_max_end && SA[block_end] != 0) { block_end++; }
                ptrdiff_t block_size    = block_end - block_start;
...
static void libsais_final_sorting_scan_left_to_right_32s_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT induction_bucket, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
...
static void libsais_final_sorting_scan_right_to_left_32s(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT induction_bucket, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
...
static void libsais_final_sorting_scan_right_to_left_32s_block_gather(const int * RESTRICT T, int * RESTRICT SA, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start, j = omp_block_start + omp_block_size - prefetch_distance - 1; i < j; i += 2)
    {
        ...
        int symbol0 = INT_MIN, p0 = SA[i + 0]; SA[i + 0] = p0 & INT_MAX; if (p0 > 0) { p0--; cache[i + 0].index = p0 | (((T[p0 - (p0 > 0)] > T[p0])) << (INT_BIT - 1)); symbol0 = T[p0]; } cache[i + 0].symbol = symbol0;
        int symbol1 = INT_MIN, p1 = SA[i + 1]; SA[i + 1] = p1 & INT_MAX; if (p1 > 0) { p1--; cache[i + 1].index = p1 | (((T[p1 - (p1 > 0)] > T[p1])) << (INT_BIT - 1)); symbol1 = T[p1]; } cache[i + 1].symbol = symbol1;
    }

    for (j += prefetch_distance + 1; i < j; i += 1)
    {
        int symbol = INT_MIN, p = SA[i]; SA[i] = p & INT_MAX; if (p > 0) { p--; cache[i].index = p | (((T[p - (p > 0)] > T[p])) << (INT_BIT - 1)); symbol = T[p]; } cache[i].symbol = symbol;
    }
}

static void libsais_final_sorting_scan_right_to_left_32s_block_sort(const int * RESTRICT T, int * RESTRICT induction_bucket, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t omp_block_start, ptrdiff_t omp_block_size)
{
    const ptrdiff_t prefetch_distance = 32;
    ...
    for (i = omp_block_start + omp_block_size - 1, j = omp_block_start + prefetch_distance + 1; i >= j; i -= 2)
    {
        ...
        {
            cache[i - 0].symbol = --induction_bucket[v0];
            if (cache[i - 0].symbol >= omp_block_start) { int ni = cache[i - 0].symbol, np = cache[i - 0].index; cache[i - 0].index = np & INT_MAX; if (np > 0) { np--; cache[ni].index = np | (((T[np - (np > 0)] > T[np])) << (INT_BIT - 1)); cache[ni].symbol = T[np]; } }
        }
        ...
        {
            cache[i - 1].symbol = --induction_bucket[v1];
            if (cache[i - 1].symbol >= omp_block_start) { int ni = cache[i - 1].symbol, np = cache[i - 1].index; cache[i - 1].index = np & INT_MAX; if (np > 0) { np--; cache[ni].index = np | (((T[np - (np > 0)] > T[np])) << (INT_BIT - 1)); cache[ni].symbol = T[np]; } }
        }
    }
    ...
    for (j -= prefetch_distance + 1; i >= j; i -= 1)
    {
        ...
        {
            cache[i].symbol = --induction_bucket[v];
            if (cache[i].symbol >= omp_block_start) { int ni = cache[i].symbol, np = cache[i].index; cache[i].index = np & INT_MAX; if (np > 0) { np--; cache[ni].index = np | (((T[np - (np > 0)] > T[np])) << (INT_BIT - 1)); cache[ni].symbol = T[np]; } }
        }
    }
}
...
static void libsais_final_sorting_scan_right_to_left_32s_block_omp(const int * RESTRICT T, int * RESTRICT SA, int * RESTRICT buckets, LIBSAIS_THREAD_CACHE * RESTRICT cache, ptrdiff_t block_start, ptrdiff_t block_size, int threads)
{
#if defined(_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && block_size >= 16384)
#endif
...
static int libsais_final_bwt_scan_right_to_left_8u_omp(const unsigned char * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT induction_bucket, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    ...
    else
    {
        ...
        for (block_start = (ptrdiff_t)n - 1; block_start >= 0; )
        {
            ...
            {
                ptrdiff_t block_max_end = block_start - ((ptrdiff_t)threads) * (LIBSAIS_PER_THREAD_CACHE_SIZE - 16 * (ptrdiff_t)threads); if (block_max_end < 0) { block_max_end = -1; }
                ptrdiff_t block_end     = block_start - 1; while (block_end > block_max_end && SA[block_end] != 0) { block_end--; }
                ptrdiff_t block_size    = block_start - block_end;
...
static void libsais_final_sorting_scan_right_to_left_8u_omp(const unsigned char * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT induction_bucket, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    ...
    else
    {
        ...
        for (block_start = (ptrdiff_t)n - 1; block_start >= 0; )
        {
            ...
            {
                ptrdiff_t block_max_end = block_start - ((ptrdiff_t)threads) * (LIBSAIS_PER_THREAD_CACHE_SIZE - 16 * (ptrdiff_t)threads); if (block_max_end < -1) { block_max_end = -1; }
                ptrdiff_t block_end     = block_start - 1; while (block_end > block_max_end && SA[block_end] != 0) { block_end--; }
                ptrdiff_t block_size    = block_start - block_end;
...
static void libsais_final_sorting_scan_right_to_left_32s_omp(const int * RESTRICT T, int * RESTRICT SA, int n, int * RESTRICT induction_bucket, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    if (threads == 1 || n < 65536)
    {
...
static void libsais_induce_final_order_32s_6k(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
...
static void libsais_induce_final_order_32s_4k(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
...
static void libsais_induce_final_order_32s_2k(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
...
static void libsais_induce_final_order_32s_1k(const int * RESTRICT T, int * RESTRICT SA, int n, int k, int * RESTRICT buckets, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    libsais_count_suffixes_32s(T, n, k, buckets);
    libsais_initialize_buckets_start_32s_1k(k, buckets);
...
static int libsais_main_32s(int * RESTRICT T, int * RESTRICT SA, int n, int k, int fs, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    if (k > 0 && fs / k >= 6)
    {
        ...
        int m = libsais_count_and_gather_lms_suffixes_32s_4k_omp(T, SA, n, k, buckets, threads, thread_state);
        if (m > 1)
        {
    ...
    else if (k > 0 && fs / k >= 4)
    {
        ...
        int m = libsais_count_and_gather_lms_suffixes_32s_2k_omp(T, SA, n, k, buckets, threads, thread_state);
        if (m > 1)
        {
    ...
    else if (k > 0 && fs / k >= 2)
    {
        ...
        if (m > 1)
        {
            ...
            int names = libsais_renumber_and_mark_distinct_lms_suffixes_32s_1k_omp(T, SA, n, m, threads);
            if (names < m)
            {
                int f = libsais_compact_lms_suffixes_32s_omp(T, SA, n, m, fs, threads, thread_state);
...
static int libsais_main_8u(const unsigned char * T, int * SA, int n, int * RESTRICT buckets, int bwt, int fs, int threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    ...
    return libsais_induce_final_order_8u_omp(T, SA, n, bwt, buckets, threads, thread_state);
}

static int libsais_main(const unsigned char * T, int * SA, int n, int bwt, int fs, int threads)
{
    LIBSAIS_THREAD_STATE *  RESTRICT thread_state   = threads > 1 ? libsais_alloc_thread_state(threads) : NULL;
    int *                   RESTRICT buckets        = (int *)libsais_alloc_aligned(8 * ALPHABET_SIZE * sizeof(int), 4096);

    int index = buckets != NULL && (thread_state != NULL || threads == 1)
        ? libsais_main_8u(T, SA, n, buckets, bwt, fs, threads, thread_state)
        : -2;
...
static int libsais_main_ctx(const LIBSAIS_CONTEXT * ctx, const unsigned char * T, int * SA, int n, int bwt, int fs)
{
    return ctx != NULL && (ctx->buckets != NULL && (ctx->thread_state != NULL || ctx->threads == 1))
        ? libsais_main_8u(T, SA, n, ctx->buckets, bwt, fs, (int)ctx->threads, ctx->thread_state)
        : -2;
}
...
int libsais(const unsigned char * T, int * SA, int n, int fs)
{
    ...
    return libsais_main(T, SA, n, 0, fs, 1);
}
...
int libsais_ctx(const void * ctx, const unsigned char * T, int * SA, int n, int fs)
{
    ...
    return libsais_main_ctx((const LIBSAIS_CONTEXT *)ctx, T, SA, n, 0, fs);
}
...
int libsais_bwt(const unsigned char * T, unsigned char * U, int * A, int n, int fs)
{
    ...
    int index = libsais_main(T, A, n, 1, fs, 1);
    if (index >= 0) 
    { 
        U[0] = T[n - 1];
...
int libsais_bwt_ctx(const void * ctx, const unsigned char * T, unsigned char * U, int * A, int n, int fs)
{
    ...
    int index = libsais_main_ctx((const LIBSAIS_CONTEXT *)ctx, T, A, n, 1, fs);
    if (index >= 0) 
    { 
        U[0] = T[n - 1];

#if defined(_OPENMP)
        libsais_bwt_copy_8u_omp(U + 1, A, index, (int)((const LIBSAIS_CONTEXT *)ctx)->threads);
        libsais_bwt_copy_8u_omp(U + 1 + index, A + 1 + index, n - index - 1, (int)((const LIBSAIS_CONTEXT *)ctx)->threads);
#else
        libsais_bwt_copy_8u(U + 1, A, index);
        libsais_bwt_copy_8u(U + 1 + index, A + 1 + index, n - index - 1);
#endif
...
int libsais_omp(const unsigned char * T, int * SA, int n, int fs, int threads)
{
    ...
    return libsais_main(T, SA, n, 0, fs, threads);
}
...
int libsais_bwt_omp(const unsigned char * T, unsigned char * U, int * A, int n, int fs, int threads)
{
    ...
    int index = libsais_main(T, A, n, 1, fs, threads);
    if (index >= 0)
    {
        U[0] = T[n - 1];
