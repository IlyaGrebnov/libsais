...
static sa_sint_t libsais64_induce_final_order_8u_omp(const uint8_t * RESTRICT T, sa_sint_t * RESTRICT SA, sa_sint_t n, sa_sint_t bwt, sa_sint_t * RESTRICT buckets, sa_sint_t threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    if (bwt)
    {
...
static sa_sint_t libsais64_main_8u(const uint8_t * T, sa_sint_t * SA, sa_sint_t n, sa_sint_t * RESTRICT buckets, sa_sint_t bwt, sa_sint_t fs, sa_sint_t threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state)
{
    ...
    return libsais64_induce_final_order_8u_omp(T, SA, n, bwt, buckets, threads, thread_state);
}

static sa_sint_t libsais64_main(const uint8_t * T, sa_sint_t * SA, sa_sint_t n, sa_sint_t bwt, sa_sint_t fs, sa_sint_t threads)
{
    ...
    sa_sint_t index = buckets != NULL && (thread_state != NULL || threads == 1)
        ? libsais64_main_8u(T, SA, n, buckets, bwt, fs, threads, thread_state)
        : -2;
...
int64_t libsais64(const uint8_t * T, int64_t * SA, int64_t n, int64_t fs)
{
    ...
    {
        return libsais64_main(T, SA, n, 0, fs, 1);
    }
}
...
int64_t libsais64_bwt(const uint8_t * T, uint8_t * U, int64_t * A, int64_t n, int64_t fs)
{
    ...
    {
        sa_sint_t index = libsais64_main(T, A, n, 1, fs, 1);
        if (index >= 0)
        {
            U[0] = T[n - 1];
            libsais64_bwt_copy_8u(U + 1, A, index);
            libsais64_bwt_copy_8u(U + 1 + index, A + 1 + index, n - index - 1);
...
int64_t libsais64_omp(const uint8_t * T, int64_t * SA, int64_t n, int64_t fs, int64_t threads)
{
    ...
    else
    {
        ...
        return libsais64_main(T, SA, n, 0, fs, threads);
    }
}
...
int64_t libsais64_bwt_omp(const uint8_t * T, uint8_t * U, int64_t * A, int64_t n, int64_t fs, int64_t threads)
{
    ...
    else
    {
        ...
        sa_sint_t index = libsais64_main(T, A, n, 1, fs, threads);
        if (index >= 0)
        {
            U[0] = T[n - 1];
            libsais64_bwt_copy_8u_omp(U + 1, A, index, threads);
            libsais64_bwt_copy_8u_omp(U + 1 + index, A + 1 + index, n - index - 1, threads);
